...
public class PoleClimbing {
    ...
    public static double climbOffset = 0.3;
    ...
    public static void main(MinecraftClient client, KeyBinding grabKey) {
        ...
        if (setPole) {
            ...
            if (client.player.getWorld().getBlockState(client.player.getBlockPos()).getBlock() != BlockInit.POLE_X && client.player.getWorld().getBlockState(client.player.getBlockPos()).getBlock() != BlockInit.POLE_Y && client.player.getWorld().getBlockState(client.player.getBlockPos()).getBlock() != BlockInit.POLE_Z) {
                touchingPole = false;
                climbing = false;
                set = false;
                verticalSet = false;
        ...
        if (grabKey.isPressed() && !climbing && setPole && !jumped && touchingPole && pole != null && client.player.getBlockStateAtPos().getBlock() == BlockInit.POLE_Y && verticalSet) {
            jumped = true;
            climbing = true;
        } else if (grabKey.isPressed() && !climbing && setPole && !jumped && touchingPole && pole != null && !verticalSet && client.world.getBlockState(pole).getBlock() != BlockInit.POLE_Y) {
            jumped = true;
            climbing = true;
        ...
        if (climbing && client.player != null && touchingPole) {
            if (pole != null) {
                if (verticalSet) {
                    if (client.world.getBlockState(pole.add(0, 1, 0)).getBlock() != BlockInit.POLE_Y && client.player.getBlockPos().getY() > pole.getY()) {
                        touchingPole = false;
                        climbing = false;
                        set = false;
                        verticalSet = false;
                    ...
                    if (climbRotation == 0 && pole != null) {
                        if (Arrays.stream(nonColidables).anyMatch(client.player.getWorld().getBlockState(pole.add(new Vec3i(0, 0, 1))).getBlock()::equals) &&
                                Arrays.stream(nonColidables).anyMatch(client.player.getWorld().getBlockState(pole.add(new Vec3i(0, 1, 1))).getBlock()::equals)) {
                        ...
                        if (Arrays.stream(nonColidables).anyMatch(client.player.getWorld().getBlockState(pole.add(new Vec3i(1, 0, 0))).getBlock()::equals) &&
                                Arrays.stream(nonColidables).anyMatch(client.player.getWorld().getBlockState(pole.add(new Vec3i(1, 1, 0))).getBlock()::equals)) {
                    ...
                    if (client.options.forwardKey.isPressed() && client.player.getPitch() < 8 && client.player.getPos().getY() <= pole.toCenterPos().getY() + 0.5 && client.world.getBlockState(client.player.getBlockPos().add(0,1,0)).getBlock() == BlockInit.POLE_Y) {
                    ...
                    if (client.options.leftKey.wasPressed()) {
                    ...
                    if (client.options.rightKey.wasPressed()) {
                        if (Arrays.stream(nonColidables).anyMatch(client.player.getWorld().getBlockState(pole.add(new Vec3i(0, 1, 0))).getBlock()::equals) &&
                                Arrays.stream(nonColidables).anyMatch(client.player.getWorld().getBlockState(pole.add(new Vec3i(0, 2, 0))).getBlock()::equals)) {
                            climbRotation = 0;
