...
public class PoleClimbing {
    ...
    public static void main(MinecraftClient client, KeyBinding grabKey) {
        ...
        if (setPole) {
            ...
            if (client.player.getWorld().getBlockState(pole).getBlock() != BlockInit.POLE_X && client.player.getWorld().getBlockState(pole).getBlock() != BlockInit.POLE_Y && client.player.getWorld().getBlockState(pole).getBlock() != BlockInit.POLE_Z) {
                touchingPole = false;
                climbing = false;
                set = false;
                verticalSet = false;
        ...
        if (climbing && client.player != null && touchingPole) {
            if (pole != null) {
                if (verticalSet) {
                    ...
                    if (client.options.leftKey.wasPressed() && touchingPole) {
                    ...
                    if (client.options.rightKey.wasPressed() && touchingPole) {
                        if (climbRotation > 0) {
                            climbRotation -= 90;
